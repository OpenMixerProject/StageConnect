...
#include "types.h"
 ...
 void Ca2b_bus_242x::ArduinoI2CWrite(U8 a2b_id, U8 I2C_address, U8 data) {


  Wire.beginTransmission(a2b_id); // transmit to device #4
  Wire.write(I2C_address);        // sends five bytes
  Wire.write(data);              // sends one byte
  Wire.endTransmission();    // stop transmitting
}

 void Ca2b_bus_242x::ArduinoI2CWrite_array( U8 a2b_id, U8 I2C_address, U8 * data, U8 len) {
  Wire.beginTransmission(a2b_id); // transmit to device #4
  Wire.write(I2C_address);        // sends five bytes
  for (int i=0; i<len; i++) {
    Wire.write(data[i]);
 ...
 int  Ca2b_bus_242x::ArduinoI2CRead (U8 a2b_id, U8 I2C_address, U8 * returndata) {
    Wire.beginTransmission(a2b_id);
    Wire.write(I2C_address);  // Register #5 address
    Wire.endTransmission();
    Wire.requestFrom(a2b_id, 1);
    * returndata= Wire.read();
    return A2B_NO_ERROR;
}

 int  Ca2b_bus_242x::ArduinoI2CRead_array (U8 a2b_id, U8 I2C_address, U8 * returndata, U8 len) {
    Wire.beginTransmission(a2b_id);
    Wire.write(I2C_address);  // Register #5 address
    Wire.endTransmission();
    Wire.requestFrom(a2b_id, len);
    for (int i=0; i<len; i++) {
...
void Ca2b_bus_242x::reset_bus(void)
{
  ...
  ArduinoI2CWrite(I2C_master, AD242X_CONTROL_REG, control_reg);
  control_reg &= ~AD242X_CONTROL_SOFTRST;

  ArduinoI2CWrite(I2C_master, AD242X_CONTROL_REG, control_reg);


  ArduinoI2CWrite(I2C_master, AD242X_SWCTL_REG,  0);  
...
int Ca2b_bus_242x::is_master_running(void)
{
  ...
  if((temp_byte == AD2425_INTTYPE_MSTR_RUNNING)){
    ...
    ArduinoI2CWrite(I2C_master, AD242X_INTMSK0_REG, 0xff);
    //ArduinoI2CWrite(I2C_master, AD242X_INTMSK1_REG, 0xff);
    ArduinoI2CWrite(I2C_master, AD242X_INTMSK2_REG, 0x0f);

    //???
    ArduinoI2CWrite(I2C_master, AD242X_INTPND2_REG, AD242X_INTPND2_DSCDONE);

    //write number of response cycles
    ArduinoI2CWrite(I2C_master, AD242X_RESPCYCS_REG, respnd_cycles_);
    ...
    ArduinoI2CWrite(I2C_master, AD242X_I2SGCFG_REG, AD242X_I2SGCFG_EARLY | AD242X_I2SGCFG_TDM8);

    //write switch enable switch
    ArduinoI2CWrite(I2C_master, AD242X_SWCTL_REG,  (A2B_SW_MODE_DEF << AD242X_SWCTL_MODE_POS) | AD242X_SWCTL_ENSW );
...
U8 Ca2b_bus_242x::irq_event(U8* event_id)
{
  ...
  //read IRQ bit
  ArduinoI2CRead(I2C_master, AD242X_INTSTAT_REG , &temp_byte);
  if (!temp_byte) {
    *event_id = 0;
    return source;
  }
  //find out int source
  ArduinoI2CRead(I2C_master, AD242X_INTSRC_REG , &temp_byte);
  ...
  ArduinoI2CRead(I2C_master, AD242X_INTTYPE_REG , &temp_byte);
...
int Ca2b_bus_242x::discover_1st_node(bool start)
{
  ...
  if(start){
    ...
    ArduinoI2CWrite(I2C_master, AD242X_DISCVRY_REG, respnd_cycles_);
  ...
  if((temp_byte == AD2425_INTTYPE_DSCDONE)){
    ...
    ArduinoI2CWrite(I2C_master, AD242X_NODEADR_REG, node_id & AD242X_NODEADR_NONE_MSK); // | AD242X_NODEADR_PERI


    //get node info
    ArduinoI2CRead(I2C_node, AD242X_VENDOR_REG, &nodes[node_id].vendor);
    ArduinoI2CRead(I2C_node, AD242X_PRODUCT_REG, &nodes[node_id].product);
    ArduinoI2CRead(I2C_node, AD242X_VERSION_REG, &nodes[node_id].version);
    ArduinoI2CRead(I2C_node, AD242X_CAPABILITY_REG, &temp_byte) ;
    ...
    ArduinoI2CWrite(I2C_node, AD242X_I2CCFG_REG, 0);


    //write switch enable switch
    ArduinoI2CWrite(I2C_node, AD242X_SWCTL_REG,  (A2B_SW_MODE_DEF << AD242X_SWCTL_MODE_POS) | AD242X_SWCTL_ENSW);
...
int Ca2b_bus_242x::discover_more_nodes(U8 node_id, bool start)
{
  ...
  if(start){
    ...
    ArduinoI2CWrite(I2C_master, AD242X_NODEADR_REG, node_pre & AD242X_NODEADR_NONE_MSK); // | AD242X_NODEADR_PERI

    //write interrupt mask to enable discovery done
    ArduinoI2CWrite(I2C_node, AD242X_INTMSK0_REG, 0x7f);
    //ArduinoI2CWrite(I2C_node, AD242X_INTMSK1_REG, 0x7f);
    ArduinoI2CWrite(I2C_node, AD242X_INTMSK2_REG, 0x0f);

    //write switch enable switch
    ArduinoI2CWrite(I2C_node, AD242X_SWCTL_REG, (A2B_SW_MODE_DEF << AD242X_SWCTL_MODE_POS) | AD242X_SWCTL_ENSW);

    //write the max number of discovery cycles
    ArduinoI2CWrite(I2C_master, AD242X_DISCVRY_REG, respnd_cycles_ - (4 * node_id));
...
int Ca2b_bus_242x::configure_node_downstream(U8 node_id)
{
  ...
  ArduinoI2CWrite(I2C_node, AD242X_DNSLOTS_REG, nodes[node_id].downslotsbypass & AD242X_DNSLOTS_MSK);
  ...
  ArduinoI2CWrite(I2C_node, AD242X_DNMASK0_REG, nodes[node_id].downslots_msk[0]);
  ArduinoI2CWrite(I2C_node, AD242X_DNMASK1_REG, nodes[node_id].downslots_msk[1]);
  ArduinoI2CWrite(I2C_node, AD242X_DNMASK2_REG, nodes[node_id].downslots_msk[2]);
  ArduinoI2CWrite(I2C_node, AD242X_DNMASK3_REG, nodes[node_id].downslots_msk[3]);
  ...
  ArduinoI2CWrite(I2C_node, AD242X_LDNSLOTS_REG, (nodes[node_id].downslots_local & AD242X_LDNSLOTS_MSK) | AD242X_LDNSLOTS_DNMASKEN );
...
int Ca2b_bus_242x::configure_node_upstream(U8 node_id)
{
  ...
  ArduinoI2CWrite(I2C_node, AD242X_UPSLOTS_REG, nodes[node_id].upslotsbypass & AD242X_UPSLOTS_MSK);
  ...
  ArduinoI2CWrite(I2C_node, AD242X_LUPSLOTS_REG, nodes[node_id].upslots_local & AD242X_LUPSLOTS_MSK);



  //Upstream mask: defines the upstream data slots which are received from the A2B bus by the local node.
  ArduinoI2CWrite(I2C_node, AD242X_UPMASK0_REG, nodes[node_id].upslots_msk[0]);
  ArduinoI2CWrite(I2C_node, AD242X_UPMASK1_REG, nodes[node_id].upslots_msk[1]);
  ArduinoI2CWrite(I2C_node, AD242X_UPMASK2_REG, nodes[node_id].upslots_msk[2]);
  ArduinoI2CWrite(I2C_node, AD242X_UPMASK3_REG, nodes[node_id].upslots_msk[3]);
...
{
  ArduinoI2CWrite(I2C_master, AD242X_DNSLOTS_REG, downstream_slots_ & AD242X_DNSLOTS_MSK);
  ArduinoI2CWrite(I2C_master, AD242X_UPSLOTS_REG, upstream_slots_ & AD242X_UPSLOTS_MSK);


  ArduinoI2CWrite(I2C_master, AD242X_SLOTFMT_REG, AD242X_SLOTFMT_DNSIZE_24 | AD242X_SLOTFMT_UPSIZE_24);
  ArduinoI2CWrite(I2C_master, AD242X_DATCTL_REG, AD242X_DATCTL_UPS | AD242X_DATCTL_DNS );
  ArduinoI2CWrite(I2C_master, AD242X_I2SRRATE_REG, 0x01 );
...
int Ca2b_bus_242x::configure_master_i2s(U8 config)
{
  ...
  ArduinoI2CWrite(I2C_master, AD242X_I2SGCFG_REG, config); //AD242X_I2SGCFG_EARLY | AD242X_I2SGCFG_TDM8

  ArduinoI2CWrite(I2C_master, AD242X_I2SCFG_REG,  AD242X_I2SCFG_TX0EN |
                      AD242X_I2SCFG_RX0EN |
                      AD242X_I2SCFG_TX1EN |
...
int Ca2b_bus_242x::configure_node_i2s(U8 node_id, U8 config, bool loopback)
{
  ...
  //set node address to node_id
  ArduinoI2CWrite(I2C_master, AD242X_NODEADR_REG, node_id & AD242X_NODEADR_NONE_MSK);

  ArduinoI2CWrite(I2C_node, AD242X_I2SGCFG_REG, config); // AD242X_I2SGCFG_EARLY | AD242X_I2SGCFG_TDM8

  ArduinoI2CWrite(I2C_node, AD242X_I2SCFG_REG, AD242X_I2SCFG_TX0EN |
                      AD242X_I2SCFG_RX0EN |
                      AD242X_I2SCFG_TX1EN |
                      AD242X_I2SCFG_TXBCLKINV |
                      AD242X_I2SCFG_RX1EN  );

  ArduinoI2CWrite(I2C_node, AD242X_PINCFG_REG, AD242X_PINCFG_DRVSTR );

  //just for debugging
  if (loopback)
    ArduinoI2CWrite(I2C_node, AD242X_I2STEST_REG, AD242X_I2STEST_BUSLOOPBK);
...
{
  U8 temp_byte = 0;
  ArduinoI2CRead(I2C_master, AD242X_VERSION_REG, &temp_byte);
...
{
  U8 temp_byte = 0;
  ArduinoI2CRead(I2C_master, AD242X_PRODUCT_REG, &temp_byte);
...
{
  U8 temp_byte = 0;
  ArduinoI2CRead(I2C_master, AD242X_VENDOR_REG, &temp_byte);
...
int Ca2b_bus_242x::node_i2c_write(U8 node_id, U8 rmt_i2c_add, U8 reg, U8* data, U8 length)
{
  ...
  ArduinoI2CWrite(I2C_node, AD242X_CHIP_REG, (rmt_i2c_add & AD242X_CHIP_MSK));
  ...
  //write data
  if (length > 1)
    ArduinoI2CWrite_array(I2C_node, reg, data , length );
  else 
    ArduinoI2CWrite(I2C_node, reg, data[0]);

  //clear NODEADR reg (MASTER)
  ArduinoI2CWrite(I2C_master, AD242X_NODEADR_REG, 0);
...
int Ca2b_bus_242x::node_i2c_read(U8 node_id, U8 rmt_i2c_add, U8 reg, U8* data, U8 length)
{
  ...
  //read data
  if (length > 1)
    status = ArduinoI2CRead_array(I2C_node, reg, data , length );
  else  
    status = ArduinoI2CRead(I2C_node, reg, data);
